...
export default {
  data() {
    ...
    return {
      ...
      total: 0, // 总条数
      // 控制对话框显示
      dialogVisible: false, // 添加证明
      editVisible: false, // 编辑证明
      // 表单数据
      addForm: {
        "communityId": 1,
        "title": "",
  ...
  methods: {
    ...
    async getActivityList() {
      // 1. 先拉取列表
      const {data: res} = await this.$http.get('/api/jiahe/proof_documents');
      if (res.code !== 200) {
      ...
      // 2. 并行拉取每条的 content，并构建新的数组（不会破坏原始对象响应性）
      const enriched = await Promise.all(records.map(async item => {
        try {
          const {data: detailRes} = await this.$http.get('/api/jiahe/proof_info', {id: item.id});
          // 返回一个新对象，包含 content
          return Object.assign({}, item, {content: detailRes?.data?.content || ''});
        } catch (err) {
          // 出错也返回原始项，避免整个 Promise.all 被 reject
          return Object.assign({}, item, {content: ''});
        }
      }));

      // 3. 一次性写入响应式数组（推荐写法）
      this.activityList = enriched;

      // 4. 等 DOM 更新后强制表格重排
      await this.$nextTick();
      if (this.$refs.activityTable && this.$refs.activityTable.doLayout) {
    ...
    handleActivityExceed(files, fileList) {
      this.$message.warning(`证明图片最多只能上传${files.length}张，已自动忽略多余文件`);
    },
    ...
    handleActivityImageRemove(file, fileList) {
      ...
      this.$message.info(`已移除证明图片："${file.name}"`);
    },
    ...
    addActivity() {
      this.$refs.addFormRef.validate(async valid => {
        ...
        const {data: res} = await this.$http.post("/api/jiahe/add_proof_info", this.addForm);
    ...
    async handleDelete(row) {
      ...
      const confirmResult = await this.$confirm('此操作将永久删除该证明, 是否继续?', '提示', {
      ...
      const {data: res} = await this.$http.delete(`/api/jiahe/delete_proof_info`,
          {
            params: {id: row.id}  // 通过 params 设置查询参数，等价于 ?id=xxx
          });

      this.$message.success("删除证明成功");
      this.getActivityList();
    },
    async handleEdit(row) {
      ...
      // console.log(info)
      /*content: "证明描述内容"id: (...)images: (...)startTime: (...)title: (...)*/
  ...
  <div>
    ...
    <el-card class="box-card">
      <div>
        ...
        <!-- 证明表格 -->
        <el-table :data="activityList" border stripe ref="activityTable">
    ...
    >
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="100px">
        ...
        <el-form-item label="详情" prop="honorDetail">
...
::v-deep .content-html {
  width: 100%; /* 继承单元格宽度 */
  min-height: 20px; /* 给一个最小高度，避免初始高度为0 */
  line-height: 1.5;
  white-space: pre-wrap; /* 保留换行 */
  word-wrap: break-word; /* 长词换行 */
  overflow: visible !important; /* 禁止溢出隐藏（覆盖表格默认样式） */
  display: inline-block; /* 确保内容撑开容器 */
}

/* 可选：调整表格单元格的默认样式 */
::v-deep .el-table__cell {
  padding: 12px 10px !important; /* 增加内边距，避免内容贴边 */
  vertical-align: top !important; /* 内容顶部对齐，避免被压缩 */
}
</style>