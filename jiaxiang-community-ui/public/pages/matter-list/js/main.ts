...
import {getList, getPages} from "./api-request.js";
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    const registEvent = function (list: MattersDAO[]) {
        const cEvent = new EventRegister();
        let cell: NavigatorCell;

        list.forEach((element: MattersDAO, index) => {
            cEvent.regist("#" + element.id,
                "click", async (e) => {
                    const detailsFragment = new DetailsPage(element, detailsTemplate)
