...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    const registEvent = function (list: Omit<ProofsDAO, "content">[]) {
        const cEvent = new EventRegister();
        let cell: NavigatorCell;

        list.forEach((element: ProofsDAO, index) => {
            cEvent.regist("#" + element.id,
                "click", async (e) => {

                    const serveInfos: Omit<ProofsDAO, "id"> = await getDetails(element.id);
